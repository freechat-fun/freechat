...
class PromptTemplateDTO(BaseModel):
    ...
    variables: Optional[Dict[str, Any]] = Field(default=None, description="Variables applied to the template, can be empty")
    format: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Prompt format: mustache (default) | f_string")
    ...
    def format_validate_regular_expression(cls, value):
        ...
        if not re.match(r"mustache|f_string", value):
            raise ValueError(r"must validate the regular expression /mustache|f_string/")
        return value
