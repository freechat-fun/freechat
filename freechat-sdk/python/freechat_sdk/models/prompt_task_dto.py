...
class PromptTaskDTO(BaseModel):
    ...
    """ # noqa: E501
    prompt_ref: PromptRefDTO = Field(description="Prompt reference information", alias="promptRef")
    ...
    params: Optional[Dict[str, Any]] = Field(default=None, description="Model call parameters")
    ...
    status: Optional[Annotated[str, Field(strict=True)]] = Field(default=None, description="Task execution status: pending | running | succeeded | failed")
    ...
    def status_validate_regular_expression(cls, value):
        ...
        if not re.match(r"pending|running|succeeded|failed", value):
            raise ValueError(r"must validate the regular expression /pending|running|succeeded|failed/")
        return value
