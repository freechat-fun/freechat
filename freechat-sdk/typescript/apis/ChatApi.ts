...
export class ChatApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getDefaultChatId(characterId: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;
     ...
     */
    public async rollbackMessagesFrom(chatId: string, messageId: number, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'chatId' is not null or undefined
        if (chatId === null || chatId === undefined) {
            throw new RequiredError("ChatApi", "rollbackMessagesFrom", "chatId");
        }


        // verify required parameter 'messageId' is not null or undefined
        if (messageId === null || messageId === undefined) {
            throw new RequiredError("ChatApi", "rollbackMessagesFrom", "messageId");
        }


        // Path Params
        const localVarPath = '/api/v1/chat/messages/rollback/from/{chatId}/{messageId}'
            .replace('{' + 'chatId' + '}', encodeURIComponent(String(chatId)))
...
export class ChatApiResponseProcessor {
     ...
     * to the expected objects
     *
     * @params response Response returned by the server for a request to rollbackMessagesFrom
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async rollbackMessagesFromWithHttpInfo(response: ResponseContext): Promise<HttpInfo<Array<number> >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: Array<number> = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
