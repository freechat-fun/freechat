...
CREATE TABLE IF NOT EXISTS `character_backend` (
  ...
  `moderation_api_key_value` text DEFAULT NULL,
  `moderation_params` json DEFAULT NULL,
  `image_model_id` varchar(98) DEFAULT NULL,
  `image_api_key_name` varchar(64) DEFAULT NULL,
  `image_api_key_value` text DEFAULT NULL,
  `image_params` json DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `chat_context` (
  ...
  `quota_type` varchar(16) DEFAULT 'none' COMMENT 'messages | tokens | none',
  `tg_user_id` bigint DEFAULT NULL COMMENT 'associated telegram user id (tg_user.tg_user_id)',
  `tg_chat_id` bigint DEFAULT NULL COMMENT 'associated telegram chat id (tg_chat.tg_chat_id)',
  `ext` json DEFAULT NULL,
  PRIMARY KEY (`chat_id`),
  INDEX `idx_user` (`user_id`),
  INDEX `idx_backend` (`backend_id`),
  INDEX `idx_tg_chat` (`tg_chat_id`),
...
CREATE TABLE IF NOT EXISTS `chat_history` (
  ...
  `ext` json DEFAULT NULL,
  `enabled` tinyint NOT NULL DEFAULT 1,
  `tg_message_id` bigint DEFAULT NULL COMMENT 'associated telegram message id (tg_message.tg_message_id)',
  `turn_id` varchar(36) DEFAULT NULL COMMENT 'turn UUID shared by input, tool exchanges and final answer; null for unreconciled legacy rows',
  `record_kind` varchar(24) NOT NULL DEFAULT 'message' COMMENT 'message | turn-start | turn-complete | turn-abort; markers delimit durable turns',
  `source_message` json DEFAULT NULL COMMENT 'original user input before prompt transformation, retained as extraction evidence',
  `message_origin` varchar(24) DEFAULT NULL COMMENT 'user-input | assistant-output | tool | template-example | system; controls evidence eligibility',
  `system_message_ref` varchar(160) DEFAULT NULL COMMENT 'chat-scoped immutable system prompt FileStore reference; no prompt body',
  `episode` bigint NOT NULL DEFAULT 0 COMMENT 'idle-delimited episode number within the chat',
  PRIMARY KEY (`id`),
  INDEX `idx_memory` (`memory_id`),
  INDEX `idx_memory_enabled` (`memory_id`, `enabled`),
  INDEX `idx_tg_message` (`tg_message_id`),
  INDEX `idx_memory_enabled_id` (`memory_id`, `enabled`, `id`),
  INDEX `idx_memory_turn_id` (`memory_id`, `turn_id`, `id`)
...
CREATE TABLE IF NOT EXISTS `chat_memory_commit` (
