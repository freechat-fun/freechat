...
export default function AzureOpenAiSettings(props: {
  ...
  const [model, setModel] = useState<AiModelInfoDTO | undefined>(
    models?.find(modelInfo => modelInfo?.modelId === (defaultParameters?.modelId ?? defaultModels.azure_open_ai)));
  
  const [baseUrl, setBaseUrl] = useState(defaultParameters?.baseUrl ?? defaultBaseURLs.azure_open_ai);
  ...
  useEffect(() => {
    setModel(models?.find(modelInfo => modelInfo?.modelId === (defaultParameters?.modelId ?? defaultModels.azure_open_ai)));
