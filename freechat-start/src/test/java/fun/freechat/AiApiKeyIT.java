...
class AiApiKeyIT extends AbstractIntegrationTest {
    ...
    void should_pass_all_tests() {
        ...
        testClient
                .get()
                .uri("/api/v2/ai/apikeys/dash_scope")
                .accept(MediaType.APPLICATION_JSON)
                .header(AUTHORIZATION, "Bearer " + apiToken)
                .exchange()
                .expectStatus()
        ...
        for (int i = 0; i < maxCount; ++i) {
            testClient
                    .get()
                    .uri("/api/v2/ai/apikey/" + ids.get(i))
                    .accept(MediaType.APPLICATION_JSON)
                    .header(AUTHORIZATION, "Bearer " + apiToken)
                    .exchange()
                    .expectStatus()
                    ...
                    .expectBody()
                    .jsonPath("$.name")
                    .isEqualTo("test_api_key_" + i)
                    .jsonPath("$.token")
                    .value(token -> assertThat(token.toString()).contains("*"));
        }

        testClient
                .get()
                .uri("/api/v2/ai/apikey/" + 1)
                .accept(MediaType.APPLICATION_JSON)
                .header(AUTHORIZATION, "Bearer " + apiToken)
                .exchange()
                .expectStatus()
        ...
        for (int i = 0; i < maxCount; ++i) {
            testClient
                    .put()
                    .uri("/api/v2/ai/apikey/disable/" + ids.get(i))
                    .accept(MediaType.APPLICATION_JSON)
                    .header(AUTHORIZATION, "Bearer " + apiToken)
                    .exchange()
                    .expectStatus()
        ...
        for (int i = 0; i < maxCount; ++i) {
            testClient
                    .put()
                    .uri("/api/v2/ai/apikey/enable/" + ids.get(i))
                    .accept(MediaType.APPLICATION_JSON)
                    .header(AUTHORIZATION, "Bearer " + apiToken)
                    .exchange()
                    .expectStatus()
