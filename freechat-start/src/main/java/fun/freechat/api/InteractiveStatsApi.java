...
public class InteractiveStatsApi {
    ...
    public Long add(
            @Parameter(description = "Info type: prompt | agent | plugin | character")
                    @PathVariable("infoType")
                    @NotBlank
                    String infoType,
            @Parameter(description = "Unique resource identifier") @PathVariable("infoId") @NotBlank String infoId,
            @Parameter(description = "Statistics type: view_count | refer_count | recommend_count | score")
                    @PathVariable("statsType")
                    @NotBlank
                    String statsType,
        ...
        return interactiveStatsService.add(
                AccountUtils.currentUser().getUserId(),
                InfoType.of(infoType),
                infoId,
                StatsType.of(statsType),
                delta == null ? 0L : delta);
    }
    ...
    public InteractiveStatsDTO get(
            @Parameter(description = "Info type: prompt | agent | plugin | character")
                    ...
                    String infoType,
            @Parameter(description = "Unique resource identifier") @PathVariable("infoId") @NotBlank String infoId) {
        return InteractiveStatsDTO.from(interactiveStatsService.get(InfoType.of(infoType), infoId));
    }
    ...
    public List<PromptSummaryStatsDTO> listPrompts(
            ...
            @Parameter(description = "Default is descending order, set asc=1 for ascending order")
                    @RequestParam("asc")
                    @Nullable
                    String asc) {
        ...
        List<InteractiveStats> statsList =
                interactiveStatsService.list(InfoType.PROMPT, StatsType.of(statsType), limit, offset, desc);
    ...
    public List<AgentSummaryStatsDTO> listAgents(
        ...
        long offset = pageNum.filter(num -> num >= 0).orElse(0L) * limit;
        List<InteractiveStats> statsList =
                interactiveStatsService.list(InfoType.AGENT, StatsType.of(statsType), limit, offset, desc);
        HashMap<Long, InteractiveStats> statsMap = new HashMap<>(statsList.size());
        User user = AccountUtils.currentUser();
        for (InteractiveStats stats : statsList) {
    ...
    public List<PluginSummaryStatsDTO> listPlugins(
        ...
        long offset = pageNum.filter(num -> num >= 0).orElse(0L) * limit;
        List<InteractiveStats> statsList =
                interactiveStatsService.list(InfoType.PLUGIN, StatsType.of(statsType), limit, offset, desc);
        HashMap<Long, InteractiveStats> statsMap = new HashMap<>(statsList.size());
        User user = AccountUtils.currentUser();
        for (InteractiveStats stats : statsList) {
    ...
    public List<CharacterSummaryStatsDTO> listCharacters(
        ...
        long offset = pageNum.filter(num -> num >= 0).orElse(0L) * limit;
        List<InteractiveStats> statsList =
                interactiveStatsService.list(InfoType.CHARACTER, StatsType.of(statsType), limit, offset, desc);
        HashMap<Long, InteractiveStats> statsMap = new HashMap<>(statsList.size());
        User user = AccountUtils.currentUser();
        for (InteractiveStats stats : statsList) {
    ...
    public List<HotTagDTO> getHotTags(
        ...
        return tagService.listHot(InfoType.of(infoType), text.orElse(null), pageSize).stream()
