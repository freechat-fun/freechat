...
public class PromptCreateDTO {
    ...
    public Triple<PromptInfo, List<String>, List<String>> toPromptInfo() {
        ...
        promptInfo.setUserId(Objects.requireNonNull(AccountUtils.currentUser()).getUserId());
        ...
        if (StringUtils.isBlank(getTemplate()) && getChatTemplate() != null) {
            try {
                promptInfo.setTemplate(InfoUtils.defaultMapper()
                        .writeValueAsString(getChatTemplate().toChatPromptContent()));
    ...
    public static PromptCreateDTO from(Triple<PromptInfo, List<String>, List<String>> promptInfoTriple) {
        ...
        if (PromptType.CHAT == PromptType.of(promptInfo.getType())) {
            try {
                ChatPromptContent promptTemplate =
                        InfoUtils.defaultMapper().readValue(promptInfo.getTemplate(), ChatPromptContent.class);
                dto.setChatTemplate(ChatPromptContentDTO.from(promptTemplate));
                dto.setTemplate(null);
            } catch (JsonProcessingException e) {
                throw new ResponseStatusException(
                        HttpStatus.BAD_REQUEST, "Invalid template: " + promptInfo.getTemplate());
            }
        }
